...
{
    public class CanUpdateEtatPointageAttribute : TypeFilterAttribute
    {
        public CanUpdateEtatPointageAttribute() : base(typeof(CanUpdateEtatPointageFilter))
        {
        }
        public class CanUpdateEtatPointageFilter : IAuthorizationFilter
        {
            private readonly ApplicationDbContext _context;
            public CanUpdateEtatPointageFilter(ApplicationDbContext context)
            {
                _context = context;
            }
            public void OnAuthorization(AuthorizationFilterContext context)
            {
                ...
                var hasPermission = _context.Modusers.Any(m =>
                    ...
                    m.Modupd == "1"         // allow update (saisie)
                );
