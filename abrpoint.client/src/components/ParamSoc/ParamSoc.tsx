...
export default function BasicTabs() {
  ...
  updateParametreMutation.mutate(dataToSend, {
    onSuccess: (response: boolean) => {
      ...
      setSnackbar({
        open: true,
        message: response ? "Paramètre mis à jour avec succès !" : "Aucune modification apportée.",
  ...
  onSuccess: () => {
    setSnackbar({
      open: true,
      message: "Mise à jour des tranches réussie",
      severity: "success",
    });
  },
  onError: () => {
      ...
      <Box sx={{ width: '100%' }} mt={-2} height={'90vh'} minWidth={'95vw'}>
        ...
        <Button
          ...
          <CustomTabPanel value={value} index={4}>
            <HeureSupp
              onChange={(data) => setHeureSuppData(data)}
