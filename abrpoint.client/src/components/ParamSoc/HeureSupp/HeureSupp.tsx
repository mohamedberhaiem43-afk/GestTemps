...
const HeureSupp: React.FC<HeureSuppProps> = ({ onChange, onChange1 }) => {
  ...
  const [socpresence, setSocpresence] = useState('P');
  const [sochsup, setSochsup] = useState('P');
  ...
  useEffect(() => {
    ...
    const trancheDataList: ParTranche[] = [
      ...tranchesH.map((tranche, index) => ({
        ...
        soccod: soccod || '', // Use the extracted soccod
        ordre: index,
      ...
      ...tranchesM.map((tranche, index) => ({
        ...
        partranche2: tranche.tranche2,
        partaux2: tranche.taux2,
        soccod: soccod || '', // Use the same soccod for consistency
        ordre: index,
        caltype: tranche.caltype,
        empreg: "M",
      }))
    ...
    socpresence,
    sochsup,
    soccod, // Add soccod to dependencies
    onChange,
    onChange1,
  ]);
  ...
  useEffect(() => {
    if (socheures) {
      setSochsup(socheures.sochsup || 'P');
      setSocpresence(socheures.socpresence || 'P');
    }
